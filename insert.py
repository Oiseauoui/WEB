...
def seed_data(connect, cur):
    ...
    def seed_disciplines():
        sql = "INSERT INTO disciplines(name, teacher_id, group_id) VALUES(?, ?, ?);"
        # Fixed the iteration over groups
        cur.executemany(sql, zip(disciplines, iter(randint(1, NUMBER_TEACHERS) for _ in range(len(disciplines))), iter(randint(1, len(groups)) for _ in range(len(disciplines)))))
...
if __name__ == '__main__':
    connect = sqlite3.connect('hw_module_6')
    cur = connect.cursor()
